...
namespace TEG.Framework.Security
{
    public class Encryption
    {
        ...
        private static byte[] Decrypt(byte[] source, byte[] key)
        {
            ...
            return DecryptTextFromMemory(source, key, null);
        }
        ...
        public static byte[] DecryptTextFromMemory(byte[] Data, byte[] Key, byte[] IV)
        {
            try
            {
                ...
                TripleDESCryptoServiceProvider tripleDESCryptoServiceProvider = new TripleDESCryptoServiceProvider();
                tripleDESCryptoServiceProvider.Mode = CipherMode.ECB;
                CryptoStream cryptoStream = new CryptoStream(stream, tripleDESCryptoServiceProvider.CreateDecryptor(Key, IV), CryptoStreamMode.Read);
                byte[] array = new byte[Data.Length];
                decimal d = array.Length / 16;
                int count = (int)Math.Ceiling(d) * 8;
                int num = cryptoStream.Read(array, 0, count);
                return array;
            }
